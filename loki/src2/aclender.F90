SUBROUTINE ACLENDER (YDCST, YDPHY, YDPHY0, KIDIA, KFDIA, KLON, KTDIAN, KLEV, PAPHI, PAPHIF, PAPRS, PAPRSF, PT, PQV, PQICE, PQLI,  &
& PNLAB, PNLABCVP, PLSCPE, PU, PV, PECT, PGZ0, PUSLE, PLEND, PPHI3)
  !-----------------------------------------------------------------------
  ! - INPUT  2D .
  ! - INPUT  1D .
  ! - OUTPUT 2D .
...
#include "fcttrm.func.h"
  ...
  ASSOCIATE (ALMAVX=>YDPHY0%ALMAVX, ACBRPHIM=>YDPHY0%ACBRPHIM, VKARMN=>YDPHY0%VKARMN, ALD=>YDPHY0%ALD, ECTMIN=>YDPHY0%ECTMIN,  &
  & ALMAVE=>YDPHY0%ALMAVE, ARSC1=>YDPHY0%ARSC1, RICRET=>YDPHY0%RICRET, ALPHAT=>YDPHY0%ALPHAT, AKN=>YDPHY0%AKN,  &
  & RFACNSM=>YDPHY0%RFACNSM, RATM=>YDCST%RATM, RCPD=>YDCST%RCPD, RCPV=>YDCST%RCPV, RCW=>YDCST%RCW, RD=>YDCST%RD,  &
  & RETV=>YDCST%RETV, RG=>YDCST%RG, RKAPPA=>YDCST%RKAPPA, RLVTT=>YDCST%RLVTT, RPI=>YDCST%RPI, RTT=>YDCST%RTT, RV=>YDCST%RV,  &
  ...
  DO JLEV=1,KLEV
    DO JLON=KIDIA,KFDIA
      ...
      ZFM_CLEAR(JLON, JLEV) =  &
      & ZOFFSET + (ZALPHAN - (ZALPHACM - ZALPHAN)*(ZSEUILM*ZARIM + (1.0_JPRB - ZSEUILM)*ATAN(ZARIM)) / ZPIS2)
      ZFH_CLEAR(JLON, JLEV) =  &
      & ZOFFSET + (ZALPHAN - (ZALPHACH - ZALPHAN)*(ZSEUILH*ZARIH + (1.0_JPRB - ZSEUILH)*ATAN(ZARIH)) / ZPIS2)
    END DO
  END DO
  ! Cas nuageux
  DO JLEV=1,KLEV
  ...
  DO JLEV=KTDIAN,KLEV
    DO JLON=KIDIA,KFDIA
      ...
      ZGZBOT(JLON) = (1.0_JPRB - PNLAB(JLON, JLEV))*ZGZBOT(JLON) + PNLAB(JLON, JLEV)*MIN(ZGZBOT(JLON), PAPHIF(JLON, JLEV))
    END DO
  END DO
  ...
  DO JLEV=KTDIAN,KLEV
    DO JLON=KIDIA,KFDIA
      ...
      ZGZBOTCVP(JLON) =  &
      & (1.0_JPRB - PNLABCVP(JLON, JLEV))*ZGZBOTCVP(JLON) + PNLABCVP(JLON, JLEV)*MIN(ZGZBOTCVP(JLON), PAPHIF(JLON, JLEV))
    END DO
  END DO
  ...
  DO JLEV=1,KLEV - 1
    !DEC$ IVDEP
    DO JLON=KIDIA,KFDIA
      ZPHI3I = 1._JPRB + MAX(ZPHI3MAX, ARSC1*MIN(2.0_JPRB, PLEND(JLON, JLEV)*PLEND(JLON, JLEV) / ZG2LD2(JLON, JLEV)))
